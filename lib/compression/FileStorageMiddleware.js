...
class FileStorageMiddleware {
  ...
  _correctPathDuplication(destFolder, baseDir) {
    try {
      ...
      // Obtenir la partie relative du chemin
      const relativePath = path.relative(normalizedBaseDir, normalizedDestFolder);
      
      if (!relativePath || relativePath === '.') {
        return baseDir;
      }
      
      // Diviser le chemin relatif en segments
      ...
      for (let i = 0; i < segments.length; i++) {
        ...
        if (currentSegment !== previousSegment) {
          ...
          console.log(`Path duplication detected and removed: ${currentSegment}`);
        }
      }
      ...
      // Reconstruire le chemin corrigé
