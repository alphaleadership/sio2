...
class FileStorageMiddleware {
  ...
  async handleDownload(req, res, next) {
    try {
      const baseDir = path.resolve("../partage");
      const reqFile = req.query.file ? path.join(baseDir, req.query.file).replace("/global/global","/global") : null;
      console.log(reqFile)
      console.log(fsSync.existsSync(reqFile))
      ...
      console.log(originalExists, compressedExists)
