...
class UploadPathResolver {
  ...
  _constructPathWithStrategy(file, destFolder, strategy) {
    try {
      ...
      // If strategy fails, try fallback strategies in order
      const fallbackStrategies = ['basename', 'webkit_path', 'smart_path'].filter(s => s !== strategy);
      
      for (const fallbackStrategy of fallbackStrategies) {
        try {
          this.logger.warn(`Strategy '${strategy}' failed, trying fallback '${fallbackStrategy}'`);
          return this._constructPathWithStrategy(file, destFolder, fallbackStrategy);
        } catch (fallbackError) {
