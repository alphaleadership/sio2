...
/**
 * Render an EJS template from the project's views directory.
 * @param {string} templatePath - Template path relative to the views directory, without the `.ejs` extension.
 * @param {Object} [data={}] - Data provided to the template during rendering.
...
let users = []; 
/**
 * Populate the module-level `users` array from the configured users JSON file.
 *
 * If the file exists and contains valid JSON, replaces `users` with the parsed value.
 * On missing file or any read/parse error, sets `users` to an empty array.
 */
function loadUsers() {
  try {
...
/**
 * Load the global compression configuration from disk and persist a canonical copy.
 *
 * Attempts to read configuration from "temp/compression-config.json" and assigns the result to
 * `global.compressionConfig`. On successful load, sets `global.compressionConfigLoadedAt` to the
 * ISO timestamp of the load and writes the configuration file back to disk (creating or updating
 * "temp/compression-config.json"). If loading or saving fails, assigns a new default
 * CompressionConfig to `global.compressionConfig` and logs the error.
 */
...
/**
 * Render the directory index template for a requested path and return the resulting HTML string.
 *
 * If rendering fails, sets `set.status` to 500 and returns a plain error message string.
 *
 * @param {object} set - Response-like object used to set status and headers for the caller.
 * @param {object} query - Request query object; may contain a `path` property specifying the requested directory.
 * @param {object|null} user - Authenticated user object to inject into the template, or `null` if unauthenticated.
 * @param {string} userDir - Absolute path to the user's root directory; used to compute relative paths for links.
 * @param {string} relBase - Base path prefix substituted for `userDir` when building links in the rendered output.
